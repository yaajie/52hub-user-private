        ...
        <div class="p-5 space-y-1">
          ...
          <div class="mt-3 pt-3 border-t theme-border">
            ...
            <router-link to="/tools" @click="showMobileMenu = false"
              class="block w-full text-left px-4 py-3 rounded-xl theme-nav-link text-sm min-h-[44px] flex items-center gap-3"
              active-class="theme-nav-link-active">
              <span class="w-5 flex items-center justify-center"><WrenchScrewdriverIcon class="h-4 w-4 opacity-70" /></span>工具集合
            </router-link>
          </div>
...
const builtinNavDefs: Record<string, { path: string; label: string; icon: string }> = {
  // 导航条专用 label（字面量，不走 i18n nav.blog；让 Footer / Blog 页 H1 / 卡片标签仍显示"资讯"）
  blog: { path: '/blog', label: 'AI 资讯', icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2' },
