...
- 微信域名风控申诉验证文件（2026-06-16，commit `9ff1292`）：
  ...
  - 生产备份：`/opt/dujiao-next/web/user.pre-wechat-verify-20260616-165450`。
  - 部署边界：只同步该 TXT 到 `/opt/dujiao-next/web/user/` 根目录；未构建、未重启、未改 admin/API/数据库/OpenResty/DNS。
  - 验证：`https://aikaitong.com/796c6b7f9381c8146af21861dd18c86d.txt` 普通 UA 与微信 `MicroMessenger` UA 均返回 `text/plain`，body 匹配。
